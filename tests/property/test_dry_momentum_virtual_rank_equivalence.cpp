...
int main() {
  try {
    ...
    bubble_cfg.radius_z_fraction = 0.2f;

    auto serial_state = dycore::make_warm_bubble_state(
        serial_domain.layout, serial_domain.metrics, bubble_cfg, "serial");
    auto split_state = dycore::make_warm_bubble_state(
        split_domain.layout, split_domain.metrics, bubble_cfg, "split");
    ...
    const auto serial_rho =
        comm::VirtualRankLayout::gather_scalar(
            ...
            serial_domain.layout, "serial_rho");
    const auto split_rho =
        comm::VirtualRankLayout::gather_scalar(
            ...
            split_domain.layout, "split_rho");

    const auto serial_theta =
        comm::VirtualRankLayout::gather_scalar(
            [&serial_state]() {
              std::vector<state::Field3D<real>> fields;
              for (const auto& s : serial_state) {
            ...
            serial_domain.layout, "serial_theta");
    const auto split_theta =
        comm::VirtualRankLayout::gather_scalar(
            [&split_state]() {
              std::vector<state::Field3D<real>> fields;
              for (const auto& s : split_state) {
            ...
            split_domain.layout, "split_theta");
    ...
    const auto serial_u = comm::VirtualRankLayout::gather_face(
        ...
        serial_domain.layout, state::FaceOrientation::X, "serial_u");
    const auto split_u = comm::VirtualRankLayout::gather_face(
        ...
        split_domain.layout, state::FaceOrientation::X, "split_u");
    ...
    const auto serial_v = comm::VirtualRankLayout::gather_face(
        ...
        serial_domain.layout, state::FaceOrientation::Y, "serial_v");
