...
void compare_face(const gwm::state::FaceField<gwm::real>& a,
  ...
  TEST_CHECK(sa.nx() == sb.nx());
  TEST_CHECK(sa.ny() == sb.ny());
  TEST_CHECK(sa.nz() == sb.nz());
...
int main() {
  ...
  for (const auto orientation : {state::FaceOrientation::X,
    ...
    comm::HaloExchange::exchange_face(oracle, layout);
    ...
    comm::HaloExchange::exchange_face(local, desc, context);
