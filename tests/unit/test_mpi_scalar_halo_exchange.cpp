...
int main() {
  ...
  auto local = comm::VirtualRankLayout::scatter_scalar(
      {desc},
  ...
  comm::HaloExchange::exchange_scalar(local.front(), desc, context);
  compare_field(local.front(), oracle_fields[static_cast<std::size_t>(context.world_rank)],
                1.0e-6);
