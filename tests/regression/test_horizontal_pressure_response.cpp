...
int main() {
  try {
    ...
    auto states = dycore::make_warm_bubble_state(domain.layout, domain.metrics,
                                                 bubble_cfg, "pressure_response");
    ...
    TEST_NEAR(initial.total_dry_mass, final.total_dry_mass, 1.0e-2);
    TEST_CHECK(final.u_face.max > 1.0e-3);
    TEST_CHECK(final.u_face.min < -1.0e-3);
    TEST_CHECK(final.v_face.max > 1.0e-3);
    TEST_CHECK(final.v_face.min < -1.0e-3);
    TEST_NEAR(final.total_horizontal_momentum_x, 0.0, 1.0e-2);
    TEST_NEAR(final.total_horizontal_momentum_y, 0.0, 1.0e-2);
